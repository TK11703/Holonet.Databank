...
namespace Holonet.Databank.API.Endpoints.Characters.UpdateRecord;

public class UpdateRecordToCharacter : IEndpoint
{
	...
	protected virtual async Task<Results<Ok<bool>, ProblemHttpResult>> Handle(int id, int recordId, UpdateRecordDto itemModel, IDataRecordService dataRecordService, IAuthorService authorService)
	{
		try
		{
			...
			var record = new DataRecord
			{
				...
				IsProcessing = itemModel.IsProcessing,
				IsProcessed = itemModel.IsProcessed,
				SystemMessage = itemModel.SystemMessage,
