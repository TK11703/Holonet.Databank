...
namespace Holonet.Databank.API.Endpoints.Planets.RecordExists;

public class PlanetRecordExists : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet($"/Planets/{{id}}/Record", HandleAsync)
    ...
    protected virtual async Task<Results<Ok<bool>, ProblemHttpResult>> HandleAsync(int id, [FromQuery] string? shard, IDataRecordService dataRecordService)
    {
        ...
        try
        {
            var exists = await dataRecordService.RecordExists(shard, planetId: id);
